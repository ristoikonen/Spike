...
namespace SpikeDetection
{
    internal static class Program
   {
        ...
        private static string DatasetRelativePath2 = $"{BaseDatasetsRelativePath}/popshort.csv";

        private static string DatasetPath = GetAbsolutePath(DatasetRelativePath);
        private static string DatasetPath2 = GetAbsolutePath(DatasetRelativePath2);
        ...
        static void Main()
        {
            ...
            IDataView dataView2 = mlContext.Data.LoadFromTextFile<PopData>(path: DatasetPath2, hasHeader: true, separatorChar: ',');
            
            //TODO: Continue from here.
            /*
            DetectSpike2(18, dataView2);

            DetectChangepoint2(18, dataView2);
            */
        ...
        static void DetectSpike2(int size, IDataView dataView)
        {
            Console.WriteLine("===============Detect temporary changes in pattern===============");

            //STEP 1: Create Esimtator   
            var estimator = mlContext.Transforms.DetectIidSpike(
                outputColumnName: nameof(PopPrediction.Prediction), inputColumnName: nameof(PopData.Value), confidence: 95, pvalueHistoryLength: size / 4);

            //STEP 2:The Transformed Model.
            //In IID Spike detection, we don't need to do training, we just need to do transformation. 
            //As you are not training the model, there is no need to load IDataView with real data, you just need schema of data.
            //So create empty data view and pass to Fit() method. 
            ITransformer tansformedModel = estimator.Fit(CreateEmptyDataView2());

            //STEP 3: Use/test model
            //Apply data transformation to create predictions.
            IDataView transformedData = tansformedModel.Transform(dataView);
            var predictions = mlContext.Data.CreateEnumerable<PopPrediction>(transformedData, reuseRowObject: false);
        ...
        static void DetectChangepoint2(int size, IDataView dataView)
        {
            Console.WriteLine("===============Detect Persistent changes in pattern===============");

            //STEP 1: Setup transformations using DetectIidChangePoint
            var estimator = mlContext.Transforms.DetectIidChangePoint(outputColumnName: nameof(PopPrediction.Prediction), inputColumnName: nameof(PopData.Value), confidence: 95, changeHistoryLength: size / 4);
            ...
            Console.WriteLine($"{nameof(PopPrediction.Prediction)} column obtained post-transformation.");
            Console.WriteLine("Alert\tScore\tP-Value\tMartingale value");
        ...
        private static IDataView CreateEmptyDataView2()
        {
            //Create empty DataView. We just need the schema to call fit()
            IEnumerable<PopData> enumerableData = new List<PopData>();
            var dv = mlContext.Data.LoadFromEnumerable(enumerableData);
            return dv;
        }
